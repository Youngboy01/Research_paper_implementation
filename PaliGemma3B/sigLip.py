...
class SiglipVisionTransformer(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.vision_embeddings = SiglipVisionEmbeddings(config)#extract patches and embed them
...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        #convolve the patch_size kernel over the image to get the patches and embed them. shape -> [batch_size,embed_dim,num_patches_h,num_patches_w]
        #where num_patches_h = height//patch_size and num_patches_w = width//patch_size
        patch_embeds = self.patch_embedding(pixel_values)#[batch_size,embed_dim,num_patches_h,num_patches_w]->[batch_size,embed_dim,num_patches]
        
        embeds = patch_embeds.flatten(2)
        #[batch_size,embed_dim,num_patches]->[batch_size,num_patches,embed_dim]
        embeds = embeds.transpose(1, 2)
        embeds = embeds + self.positional_embedding(self.position_ids)
        #each positional embedding is a vector of size embed_dim
...
class SiglipAttention(
    ...
    def forward(
        ...
        )  # reshape avoids computational overhead
        attn_output = self.W_o(attn_output)  # for mixing the results of the head
